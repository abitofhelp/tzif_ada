...
--
--  Purpose:
--    Windows platform I/O adapter for TZif operations.
--    Same interface as Desktop I/O but uses Windows platform adapter
--    for system timezone detection.
--
--  Key Differences from Desktop:
--    - Read_System_Timezone_Id uses Win32 API + CLDR mapping
--    - No default zoneinfo path (user must provide via TZIF_DATA_PATH env)
--
--  Dependencies:
--    Interfaces
...
package TZif.Infrastructure.IO.Windows with
  SPARK_Mode => Off
is
   ...
   --
   --  Implementation:
   --    - Maps Zone_Id to filesystem path via TZIF_DATA_PATH/{Id}
   ...
   --
   --  Reads the system's timezone ID using Win32 API + CLDR mapping.
   --
   --  Parameters:
   --    Result : Ok(Zone_Id_Type) or Error(Not_Found, IO_Error)
   --
   --  Implementation:
   --    - Calls GetDynamicTimeZoneInformation Win32 API
   --    - Maps Windows timezone name to IANA zone ID via CLDR data
   --    - Returns zone ID like "America/New_York"
   -------------------------------------------------------------------------
