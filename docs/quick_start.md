...
procedure Discover_Example is
   ...
   package Path_Strings is new Ada.Strings.Bounded.Generic_Bounded_Length (512);

   Paths : Path_List (1 .. 2);
begin
   Paths (1) := Path_Strings.To_Bounded_String ("/usr/share/zoneinfo");
   Paths (2) := Path_Strings.To_Bounded_String ("/var/db/timezone/zoneinfo");
...
```bash
cp config/profiles/baremetal/tzif_config.ads config/tzif_config.ads
alr build
```
...
Available: `standard`, `embedded`, `concurrent`, `baremetal`, `stm32h7s78`, `stm32mp135`
...
- **[Architecture Guide](guides/hybrid-architecture/index.md)** - Understand hexagonal architecture
...
- **[Test Infrastructure](guides/test_infrastructure.md)** - Learn about the test suite
...
TZif is licensed under the BSD-3-Clause License.
